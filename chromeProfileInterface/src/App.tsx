    ...
    setProfileArray(prevArray => {
      ...
      if (browserType == 'chrome') {
        newProfile = {
          ...
          folderPath: String.raw`C:\Users\Renz\Desktop\Browser Profiles`,
          pid: 0,
          delete: false
        }
      }
        ...
        <button title='Delete Profile'
          onClick={async () => {
            if (confirm(`Are you sure you want to delete Profile ${profile.name}?`)) {
              await tasks.deleteProfile(profile.folderPath + '\\' + `${profile.name}`)
              setProfileArray((item) => {
                return item.filter((user) => user.name !== profile.name)
              })
              let helperSt = new Set(availableNumbers)
              setAvailableNumbers(helperSt.add(parseInt(profile.name)))
            }
      ...
      <div className='flex flex-col'>
        ...
        <div className='titleBar flex flex-row gap-1 justify-end bg-neutral-800 '>
          ...
          <div className='flex grow justify-center'>
            <h1 className='text-white mt-1'>Profile Interface</h1>
          </div>
          {/* settings button */}
          {/* <button className=''>
